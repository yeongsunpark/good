...
def three_checker(ind, q_id, classify, mod_question, mod_answer, mod_context):
    ...
    if classify == 1:
        if mod_question == "" and mod_answer == "" and mod_context == "":
            ...
            logger.error("Line%s, q_id:%s, mod_question:%s, mod_answer:%s, mod_context:%s", ind, q_id, mod_question, mod_answer, mod_context)
            exit()
    ...
    # 분류 1번 아닌데 체커 안 했을 경우 확인
        ...
        if mod_question != "" or mod_answer != "" or mod_context != "":
            ...
            logger.error("Line%s, q_id:%s, class:%s, m_q:%s, m_a:%s, m_c:%s", ind, q_id, classify, mod_question, mod_answer, mod_context)
            exit()
