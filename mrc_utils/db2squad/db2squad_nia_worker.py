...
class SquadDb():
    ...
    def __init__(self):
        ...
        self.data_output_dir = "/home/msl/ys/squad"
        self.lang = "no"
        self.version = "0506"
    ...
    def connect_db(self, table_name):
        try:        # try to connect to project db
            ...
            cfg_dict = dict(host='10.122.64.83', usr= 'root', pwd='data~secret!', db=table_name)
    ...
    def db2squad(self):
        ...
        fetch_sql_ctx = "SELECT id, title, context, source FROM all_context_all where season in (1, 2, 3, 6) {};".format(self.random_end)
        ...
        contextss = self.cur.fetchall()   # entire
        print (len(contextss))
        """
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as exe:
            fs = {exe.submit(cqas, contextss, n, r) for n in range(0, len(contextss), r)}

def cqas(contexts, n, r):
        con = None
        cur = None
        cfg_dict = dict(host='10.122.64.83', usr='root', pwd='data~secret!', db='SQUAD_NEWS_NIA')
        con = pymysql.connect(host=cfg_dict['host'], user=cfg_dict['usr'],
                                   passwd=cfg_dict['pwd'], db=cfg_dict['db'], charset='utf8')
        cur = con.cursor()
        ...
        logger.info("Finish creating json structure({})".format(n))

        with open(os.path.join("/home/msl/ys/squad", "ko_nia_v{}_squad{}_{}.json".
                format("0", "0", n)),
                  'w', encoding='utf8') as fp:
            json.dump(result, fp, ensure_ascii=False)
        with open(os.path.join("/home/msl/ys/squad", "ko_nia_v{}_squad_pretty{}_{}.json".
                format("0", "0", n)),
                  'w', encoding='utf8') as fp:
            json.dump(result, fp, ensure_ascii=False, indent=2)
        logger.info("Data dump {} finished..: len {}".format(n, cnt))
